def place_elements_offset(screen_width, screen_height, element_width, element_height, xoffset, yoffset):
...
def get_float_idx_from_coords(x_coord, y_coord, start_x, start_y, cell_size):
    x_pos = (x_coord - start_x) / cell_size
    y_pos = (y_coord - start_y) / cell_size
    return y_pos, x_pos  # in matrix, horizontal is columns and vertical are rows
...
def get_tiny_matrix(matrix, cell_size, pacman_speed):
    ...
    tiny_matrix = [["null"] * num_cols * sub_div for _ in range(num_rows * sub_div)]
    for r in range(num_rows):
        for c in range(num_cols):
            cell = matrix[r][c]
            # If a tile is not 'wall' or 'elec', treat it as 'null' in the tiny grid
            if cell not in ("wall", "elec"):
                cell = "null"
            for sx in range(sub_div):
                for sy in range(sub_div):
                    tiny_matrix[r * sub_div + sx][c * sub_div + sy] = cell
    return tiny_matrix

def get_movable_locations(matrix, max_cell_size=20, cell_size=20):
    movables = []
    ...
    for r_idx in range(rows):
        for c_idx in range(cols):
            if (r_idx + (subdiv * 2) <= rows) and (c_idx + (subdiv * 2) <= cols) and is_valid(r_idx, c_idx):
